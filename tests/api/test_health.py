...
def test_explain_returns_shap_contributions(client: TestClient) -> None:
    """POST /explain must return SHAP contributions and explanation text."""
    with (
        patch("backend.api.routes._load_resources", return_value=None),
        patch("backend.api.routes._ensemble", MagicMock(predict_proba=MagicMock(return_value=np.array([[0.2, 0.8]])))),
        patch("backend.api.routes._model_features", ["feat_a", "feat_b", "feat_c"]),
        patch("backend.api.routes._features", ["feat_a", "feat_b", "feat_c"]),
    ...
    body = response.json()
    assert body["feature_contributions"]["feat_a"] == 0.1
    assert body["top_features"][0]["feature"] == "feat_a"
    assert body["explanation_text"] == "explanation text"


def test_threshold_returns_metrics(client: TestClient) -> None:
    """GET /model/threshold must return summary metrics for the provided threshold."""
    with (
        patch("backend.api.routes._load_resources", return_value=None),
        patch("backend.api.routes._ensemble", MagicMock(predict_proba=MagicMock(return_value=np.array([[0.2, 0.8], [0.8, 0.2]])))),
        patch("backend.api.routes._X_test", pd.DataFrame(np.array([[0.2, 0.8], [0.8, 0.2]]), columns=["feat_a", "feat_b"])),
        patch("backend.api.routes._y_test", np.array([1, 0])),
    ):
        response = client.get("/model/threshold?threshold=0.5")

    assert response.status_code == 200, response.text
    body = response.json()
    assert body["threshold"] == 0.5
    assert body["total"] == 2
    assert body["confusion_matrix"] == [[1, 0], [0, 1]]
